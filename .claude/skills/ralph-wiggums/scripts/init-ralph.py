...
def init_ralph(project_dir: Path, project_name: str = None):
    ...
    print(f"✅ Created {ralph_dir}")
    
    # Detect stack
    stack = detect_stack(project_dir)
    print(f"✅ Detected stack: {stack['language']}/{stack['framework']}")
...
echo "🚀 Starting Ralph Wiggums"

for i in $(seq 1 $MAX_ITERATIONS); do
  if [[ -f "$SCRIPT_DIR/.ralph-pause" ]]; then
    echo "⏸️ Pause requested"
    exit 0
  fi
...
''')
    ...
    print(f"✅ Created {target_ralph_sh}")
